...
void first3_large(int *A, int *top, int *top2)
{
    ...
    if (top <= top2 - 1)
    {
        if (A[first] == largesty || A[first - 1] == largesty || A[first - 2] == largesty || A[first - 3] == largesty || A[first - 4] == largesty)
        {
            if (A[first] == largesty)
            {
                ra(A, top, top2);
...
{
    while ((sort_check(A, top, top2) == *top && *top != top_uni) || (sort_check(A, top, top2) == *top+2 && *top != top_uni))
    {
